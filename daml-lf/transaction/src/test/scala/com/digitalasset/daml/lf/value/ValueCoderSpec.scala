...
class ValueCoderSpec extends WordSpec with Matchers with EitherAssertions with PropertyChecks {
  ...
  "encode-decode" should {
    ...
    "do ContractId" in {
      forAll(coidValueGen) { v: Value[ContractId, WellTyped] =>
        testRoundTrip(v)
      }
    }
    ...
    "do lists" in {
      forAll(valueListGen) { v: ValueList[ContractId, WellTyped] =>
        testRoundTrip(v)
      }
    }

    "do optionals" in {
      forAll(valueOptionalGen) { v: ValueOptional[ContractId, WellTyped] =>
        testRoundTrip(v)
      }
    }

    "do maps" in {
      forAll(valueMapGen) { v: ValueMap[ContractId, WellTyped] =>
        testRoundTrip(v)
      }
    }

    "do variant" in {
      forAll(variantGen) { v: ValueVariant[ContractId, WellTyped] =>
        testRoundTrip(v)
      }
    }

    "do record" in {
      forAll(recordGen) { v: ValueRecord[ContractId, WellTyped] =>
        testRoundTrip(v)
      }
    }
    ...
    "do versioned value with supported override version" in forAll(valueGen, valueVersionGen) {
      (value: Value[ContractId, WellTyped], version: ValueVersion) =>
        testRoundTripWithVersion(value, version)
    }

    "do versioned value with assigned version" in forAll(valueGen) {
      v: Value[ContractId, WellTyped] =>
    ...
    "versioned value should pass serialization if unsupported override version provided" in
      forAll(valueGen, unsupportedValueVersionGen) {
        (value: Value[ContractId, WellTyped], badVer: ValueVersion) =>
    ...
    "versioned value should fail deserialization if version is not supported" in
      forAll(valueGen, unsupportedValueVersionGen) {
        (value: Value[ContractId, WellTyped], badVer: ValueVersion) =>
          ...
          val actual: Either[DecodeError, VersionedValue[ContractId, WellTyped]] =
  ...
  def testRoundTrip(value: Value[ContractId, WellTyped]): Assertion = {
    val recovered = ValueCoder.decodeValue(
      defaultCidDecode,
      defaultValueVersion,
  ...
  def testRoundTripWithVersion(
      value0: Value[ContractId, WellTyped],
      version: ValueVersion): Assertion = {
    ...
    val decoded: VersionedValue[ContractId, WellTyped] = assertRight(
    ...
    val decodedSentOverWire: VersionedValue[ContractId, WellTyped] = assertRight(
