...
trait ValueSerializer {
  def serialiseValue(value: VersionedValue[AbsoluteContractId, WellTyped])
    : Either[ValueCoder.EncodeError, Array[Byte]]

  def deserialiseValue(
      blob: Array[Byte]): Either[DecodeError, VersionedValue[AbsoluteContractId, WellTyped]]
}
...
object ValueSerializer extends ValueSerializer {
  ...
  override def serialiseValue(value: VersionedValue[AbsoluteContractId, WellTyped])
    : Either[ValueCoder.EncodeError, Array[Byte]] =
    ValueCoder
